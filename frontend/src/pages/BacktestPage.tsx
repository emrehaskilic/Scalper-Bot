    ...
    <div className="min-h-screen bg-[#09090b] text-zinc-200 p-4 md:p-6">
      <div className="max-w-7xl mx-auto space-y-5">
        ...
        <div>
          ...
          <span className="text-[11px] text-zinc-500">Historical simulation using the same signal + risk engine</span>
        </div>
        ...
        {config && (
          <div className="bg-zinc-800/50 rounded border border-zinc-700/50 p-4 space-y-4">
            ...
            {/* Strategy */}
            <div>
              <h2 className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider mb-2">Strateji</h2>
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3 text-xs">
                <label className="space-y-1">
                ...
                <label className="space-y-1">
                  <span className="text-zinc-500 text-[10px] uppercase">Multiplier</span>
                  <input type="number" step="1" min="1"
                ...
                <label className="space-y-1">
                  <span className="text-zinc-500 text-[10px] uppercase">ALMA Sigma</span>
                ...
                <label className="space-y-1">
                  <span className="text-zinc-500 text-[10px] uppercase">ALMA Offset</span>
        ...
        <div className="bg-zinc-800/50 rounded border border-zinc-700/50 p-4">
          <div className="flex items-center gap-3 mb-3 flex-wrap">
            ...
            <span className="text-[10px] text-zinc-500">{selectedSymbols.length}/50</span>
            ...
            <label className="flex items-center gap-2 text-xs">
              ...
              <input type="number" min={1} max={365} step={1}
                ...
                className="w-16 bg-zinc-900 border border-zinc-700 rounded px-2 py-1 text-sm font-mono text-zinc-200" />
              <span className="text-zinc-500">gun</span>
            </label>
            {result && !running && (
              <button onClick={handleReset}
                className="px-4 py-1.5 rounded text-xs font-semibold bg-zinc-500/20 text-zinc-300 border border-zinc-500/30 hover:bg-red-500/20 hover:text-red-400 hover:border-red-500/30 transition-colors">
                Sifirla
              </button>
            )}
            <button onClick={handleRun}
              disabled={selectedSymbols.length === 0 || running || !!result}
              className="px-4 py-1.5 rounded text-xs font-semibold bg-blue-500/20 text-blue-400 border border-blue-500/30 hover:bg-blue-500/30 transition-colors disabled:opacity-30 disabled:cursor-not-allowed">
            ...
            <div className="flex flex-wrap gap-2">
              {selectedSymbols.map((s) => (
                <button key={s} onClick={() => handleRemovePair(s)}
                  ...
                  {s} <span className="text-zinc-500 hover:text-red-400">&times;</span>
                </button>
              ))}
            </div>
          )}
        ...
        {running && (
          <div className="bg-zinc-800/50 rounded border border-zinc-700/50 p-4">
            <div className="flex items-center justify-between text-xs mb-2">
              <span className="text-zinc-400">{btStatus === "fetching" ? "Veri indiriliyor..." : btStatus === "computing" ? "Sinyaller hesaplaniyor..." : btStatus === "simulating" ? "Simulasyon calisiyor..." : "Metrikler hesaplaniyor..."}</span>
              <span className="text-zinc-500 font-mono">{progress.toFixed(0)}%</span>
            </div>
            <div className="w-full h-2 bg-zinc-700 rounded overflow-hidden">
              <div className="h-full bg-blue-500 transition-all duration-300 rounded"
        ...
        {m && (
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
              ...
              <MetricTile label="Bakiye" value={`${formatNum(m.current_balance, 2)} USDT`} color="text-blue-400" />
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
              ...
              <MetricTile label="Toplam Fee" value={`${formatNum(m.total_fees, 2)} USDT`} color="text-zinc-400" />
            </div>

            {/* ── Per-Symbol Metrics ── */}
            {result!.per_symbol && result!.per_symbol.length > 0 && (
              <div className="bg-zinc-800/50 rounded border border-zinc-700/50 p-4">
                <h2 className="text-sm font-semibold text-zinc-300 mb-3">Parite Bazinda Sonuclar</h2>
                  ...
                  <table className="w-full text-xs">
                    ...
                    <tbody>
                      {result!.per_symbol.map((ps) => (
                        <tr key={ps.symbol} className="border-b border-zinc-800/50 hover:bg-zinc-800/30">
            ...
            {result!.equity_curve.length > 0 && (
              <div className="bg-zinc-800/50 rounded border border-zinc-700/50 p-4">
                <h2 className="text-sm font-semibold text-zinc-300 mb-3">Equity Curve</h2>
                <ResponsiveContainer width="100%" height={300}>
                  <AreaChart data={result!.equity_curve}>
                    ...
                    <XAxis dataKey="time" tickFormatter={fmtDate} tick={{ fontSize: 10, fill: "#71717a" }}
            ...
            {result!.drawdown_curve.length > 0 && (
              <div className="bg-zinc-800/50 rounded border border-zinc-700/50 p-4">
                <h2 className="text-sm font-semibold text-zinc-300 mb-3">Run-up & Drawdown (%)</h2>
            ...
            {result!.trades.length > 0 && (
              <div className="bg-zinc-800/50 rounded border border-zinc-700/50 p-4">
                <h2 className="text-sm font-semibold text-zinc-300 mb-3">Islem Bazinda PnL (USDT)</h2>
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={result!.trades}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#27272a" />
                    <XAxis dataKey="id" tick={{ fontSize: 9, fill: "#71717a" }} />
                    <YAxis tick={{ fontSize: 10, fill: "#71717a" }}
                      tickFormatter={(v: number) => `${v.toFixed(1)}`} />
                    <Tooltip content={({ active, payload }: any) => {
                      ...
                      return (
                        <div className="bg-zinc-900 border border-zinc-700 rounded px-3 py-2 text-xs">
                          ...
                          <p className="text-zinc-400">{t.side} — {t.exit_reason}</p>
            ...
            {result!.trades.length > 0 && (() => {
              ...
              <div className="bg-zinc-800/50 rounded border border-zinc-700/50 p-4">
                <div className="flex items-center gap-3 mb-3">
                  <h2 className="text-sm font-semibold text-zinc-300">Islem Listesi ({filtered.length})</h2>
                        ...
                        <tr key={t.id} className="border-b border-zinc-800/50 hover:bg-zinc-800/30">
                          ...
                          <td className="py-1.5 text-center">
                            <span className={`px-1.5 py-0.5 rounded text-[10px] font-semibold ${
                              t.side === "LONG" ? "bg-emerald-400/20 text-emerald-400" : "bg-red-400/20 text-red-400"
                            }`}>{t.side}</span>
                          </td>
                          ...
                          <td className="py-1.5 px-2 text-center">
                            <span className={`px-1.5 py-0.5 rounded text-[10px] ${
                              t.exit_reason.startsWith("TP") ? "bg-emerald-400/10 text-emerald-400" :
        ...
        <div className="text-center text-[10px] text-zinc-600 pb-4">
          Scalper Bot v0.1.0 — Backtest Engine | Pine Script Strategy Port
        </div>
      </div>
    </div>
  );
